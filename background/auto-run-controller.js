(function attachBackgroundAutoRunController(root, factory) {
  ...
  function createAutoRunController(deps = {}) {
    ...
    function describeAutoRunDiagnosticState(state = {}) {
      const runningNodes = getRunningWorkflowNodes(state);
      ...
      const statusParts = Object.entries(nodeStatuses)
        .filter(([, status]) => String(status || '').trim() === 'running')
      ...
      return [
        ...
        `runningStatuses=${statusParts.length ? statusParts.join(',') : '(无)'}`,
      ].join('；');
    }

    async function runWithAutoRunHeartbeat(label, action, options = {}) {
      const intervalMs = Math.max(5000, Math.floor(Number(options.intervalMs) || 30000));
      ...
      const heartbeat = async () => {
        while (!completed) {
          ...
          try {
            ...
            await addLog(`自动运行诊断：${label} 已等待 ${elapsedSeconds} 秒；${describeAutoRunDiagnosticState(state)}`, 'warn');
          } catch (_error) {
            // Diagnostic logging must not interrupt the real auto-run action.
          }
        }
      };
        ...
        while (attemptRun <= maxAttemptsForRound) {
          ...
          try {
            ...
            if (!useExistingProgress && startNodeId === defaultStartNodeId && typeof ensureHotmailMailboxReadyForAutoRunRound === 'function') {
              await runWithAutoRunHeartbeat(
                `第 ${targetRun}/${totalRuns} 轮第 ${attemptRun} 次尝试：准备 Hotmail 邮箱会话`,
                () => ensureHotmailMailboxReadyForAutoRunRound({
                  targetRun,
                  totalRuns,
                  attemptRun,
            ...
            await runWithAutoRunHeartbeat(
              `第 ${targetRun}/${totalRuns} 轮第 ${attemptRun} 次尝试：执行节点序列（起点 ${startNodeId}）`,
              () => runAutoSequenceFromWorkflowNode(startNodeId, {
                targetRun,
                totalRuns,
