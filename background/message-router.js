  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'STOP_FLOW': {
          if (message.source === 'sidepanel' && message.payload?.trustedClick !== true) {
            await addLog('已忽略来源为侧栏但缺少真实点击标记的停止请求。', 'warn');
            return { ok: false, ignored: true, reason: 'untrusted_stop_request' };
          }
          await requestStop();
          return { ok: true };
        }
